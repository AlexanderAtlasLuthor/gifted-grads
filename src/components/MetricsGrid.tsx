  ...
  return (
    <div className="text-xs">
      <div className="flex items-center justify-between">
        <span className="truncate pr-2 uppercase tracking-widest text-ink-100/80">{label}</span>
      ...
      <div className="mt-2 h-1.5 w-full rounded-full bg-white/5">
        <div
          className="h-full rounded-full bg-gradient-to-r from-sky-400 to-accent-400"
...
export function MetricsGrid() {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-ink-100/80">
        <Spinner />
      ...
      >
        <div className="space-y-2">
          {INSURANCE_TYPES.every((k) => data.byInsuranceType[k] === 0) && (
            <div className="text-xs text-ink-200/60">—</div>
          )}
